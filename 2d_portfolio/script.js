...
document.addEventListener('DOMContentLoaded', function() {
	...
	{ 
        ...
        { 
            src: 'images/thumbnails/fr_image_14_thumbnail.jpg', 
            fullSrc: 'images/73.jpg', 
            category: ['freelance']
        },
        { 
    ...
    function displayImages(filter) {
        gallery.innerHTML = '';
        const filteredImages = filter === 'all' ? images : images.filter(img => 
            Array.isArray(img.category) ? img.category.includes(filter) : img.category === filter
        );
        filteredImages.forEach(img => {
            const imgElement = document.createElement('img');
            imgElement.src = img.src;
            imgElement.alt = Array.isArray(img.category) ? img.category.join(', ') : img.category;
